...
int main()
{
    ...
    while(T--){
        int N, M; cin >> N >> M;
        int S = 0, T = N * 2 + 1;
        MCMF<int, int, 1010> mcmf;
        for(int i=0; i<M; i++){
            int u, v; cin >> u >> v;
            mcmf.addEdge(u, v + N, mcmf.flow_max, 1);
            mcmf.addEdge(v, u + N, mcmf.flow_max, 1);
        }
        for(int i=1; i<=N; i++){
            int u; cin >> u;
            if(u) mcmf.addEdge(S, i, 1, 0);
        }
        for(int i=1; i<=N; i++){
            int u; cin >> u;
            if(u) mcmf.addEdge(i + N, T, 1, 0);
        }
        for(int i=1; i<=N; i++){
            mcmf.addEdge(i, i + N, mcmf.flow_max, 0);
            mcmf.addEdge(i + N, i, mcmf.flow_max, 0);
        }
