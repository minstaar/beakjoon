...
int main()
{
    ...
    MCMF<int, int, 2510> mcmf;
    for(int i=0; i<N; i++){
        for(int j=0; j<M; j++){
            cin >> str[i][j];
            if(str[i][j] == 'F') str[i][j] = 'E';
        }
    }
    for(int i=0; i<N; i++){
        for(int j=0; j<M; j++){
            if((i + j) % 2 == 0){
                mcmf.addEdge(S, i * M + j, 1, 0);
                mcmf.addEdge(i * M + j, T, 1, 0);
                for(int k=0; k<4; k++){
                    int ny = i + "1210"[k] - '1';
                    int nx = j + "2101"[k] - '1';
                    if(ny < 0 || ny >= N || nx < 0 || nx >= M) continue;
                    mcmf.addEdge(i * M + j, ny * M + nx, 1, -cost[str[i][j] - 'A'][str[ny][nx] - 'A']);
                }
            }
            else mcmf.addEdge(i * M + j, T, 1, 0);
        }
    }
